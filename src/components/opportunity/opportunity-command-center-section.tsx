...
export function OpportunityCommandCenterSection({
  title,
  description,
  count,
  href,
  ...
  children: ReactNode;
  emptyMessage: string;
  accent?: "default" | "risk" | "growth" | "revenue";
}) {
  const accents = {
    default: "border-carbon-blue/8",
    risk: "border-red-500/20",
    growth: "border-emerald-500/20",
    revenue: "border-violet-500/20",
  } as const;

  return (
    <section
      className={`dashboard-card flex flex-col overflow-hidden border-t-2 ${accents[accent]}`}
    >
