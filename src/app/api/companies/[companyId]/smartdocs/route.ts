...
async function parseCreateSmartDocRequest(request: Request): Promise<{
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    const LinkedProjectId =
      String(form.get("LinkedProjectId") ?? "").trim() || undefined;
...
export async function GET(
  ...
  try {
    ...
    if (
      ...
      identityPreview = buildCompanySmartDocIdentityPreview(
        context.companyCode,
        context.companyName,
...
export async function POST(
  ...
  if (!canUploadSmartDocs(role)) {
    return sharePointErrorResponse(
      SharePointServiceError.forbidden(
        "You cannot upload SmartDocs for this company",
      ),
    );
  }
  ...
  try {
    ...
    // Explicit reject: do not accept DocumentSetID on company path (PI/BQ stuffing).
    if ((metadata as CreateSmartDocInput & { DocumentSetID?: string }).DocumentSetID) {
      return NextResponse.json(
        {
          error:
            "Company-owned SmartDocs cannot join opportunity Document Sets (PI/BQ/FQ)",
        },
        { status: 400 },
      );
    }

    const imported = await importCompanySmartDoc({
      companyId,
      metadata: {
