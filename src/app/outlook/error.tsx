...
/**
 * App Router error UI for /outlook/* task panes and dialogs.
 */
export default function OutlookError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[SmartCRM /outlook]", error);
  }, [error]);
    ...
    <div className="flex h-[100dvh] flex-col justify-center bg-white px-6">
      <div className="w-full max-w-sm border border-carbon-blue/10 bg-carbon-blue/[0.02] p-5">
        ...
        <p className="mt-1 text-[11px] leading-relaxed text-carbon-blue/50">
          The Outlook pane hit a temporary error. Sign in to continue.
        </p>
        <button
          type="button"
          onClick={() => {
