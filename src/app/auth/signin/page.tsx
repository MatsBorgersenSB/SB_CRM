        ...
        <div className="w-full max-w-md border border-white/10 bg-white/[0.04] p-8 backdrop-blur-sm sm:p-10">
          ...
          >
            {pending ? (
              <>
                <span
                  className="inline-block h-3.5 w-3.5 animate-spin border border-white border-t-transparent"
                  aria-hidden
                />
          ...
          <p className="mt-4 text-center text-[11px] text-white/35">
            Authorized Standard Bio tenants only. Sessions use Microsoft Entra ID.
          </p>
        </div>
      </main>

      <footer className="relative z-10 px-6 py-4 text-[10px] text-white/30 sm:px-10">
        © {new Date().getFullYear()} Standard Bio · SmartCRM
      </footer>
    </div>
  );
}
