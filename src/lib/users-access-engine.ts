    ...
    if (!hasCompanyOwner(company)) {
      gaps.push(
        buildGap({
          ...
          title: `${company.Title} has no account owner`,
          why: "Every company requires a named Standard Bio account owner.",
          impact: "SmartAssist cannot route attention; commercial accountability is unclear.",
          recommendedAction: "Assign an account owner from Users & Access or the company record.",
          severity: "critical",
    ...
    if (!activity.ActivityOwner?.Title?.trim()) {
      gaps.push(
        buildGap({
          ...
          recommendedAction: "Assign an activity owner from the deal team or account owner.",
          severity: "warning",
